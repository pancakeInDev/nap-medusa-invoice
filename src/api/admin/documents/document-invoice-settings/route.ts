...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    const lastDocumentInvoiceSettings =
      await documentsModuleService.listDocumentInvoiceSettings(
        {},
        {
          order: {
            created_at: "DESC",
    ...
    res.status(200).json({
      settings:
        lastDocumentInvoiceSettings && lastDocumentInvoiceSettings.length
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    const newSettings = await documentsModuleService.updateInvoiceSettings(
      formatNumber,
      forcedNumber,
